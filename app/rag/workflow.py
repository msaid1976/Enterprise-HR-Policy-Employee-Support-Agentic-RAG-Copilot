...
def _build_openai_llm():
    if not settings.openai_api_key:
        raise RuntimeError(
            ...
            "Set OPENAI_API_KEY in your .env, or set LLM_PROVIDER=groq."
        )
    return ChatOpenAI(
...
def _build_groq_llm():
    if not settings.groq_api_key:
        raise RuntimeError(
            ...
            "Set GROQ_API_KEY in your .env, or set LLM_PROVIDER=openai."
        )
    return ChatGroq(
...
# Registry of LLM providers. Add new entries here to support more chat
# models without touching llm() itself.
LLM_PROVIDER_BUILDERS = {
...
def llm():
    ...
    if _llm is None:
        ...
        builder = LLM_PROVIDER_BUILDERS.get(provider)
        if builder is None:
            raise ValueError(
                f"Unsupported LLM_PROVIDER '{settings.llm_provider}'. "
                f"Choose one of: {', '.join(LLM_PROVIDER_BUILDERS)}"
            )
